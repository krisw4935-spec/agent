...
async def lifespan(app: FastAPI):
    ...
    # Seed default usable student account
    try:
        default_email = "student@math-teacher.local"
        user = await database_service.get_user_by_email(default_email)
        if user is None:
            await database_service.create_user(
                email=default_email,
                password=User.hash_password("Math123456!"),
                username="示例学生",
            )
            logger.info("default_student_user_created", email=default_email)
    except Exception as e:
        logger.exception("default_student_user_seed_failed", error=str(e))
