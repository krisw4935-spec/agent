...
class DatabaseService:
    ...
    def __init__(self):
        ...
        # Ensure all tables (including evolution tables) exist
        try:
            SQLModel.metadata.create_all(self.engine)
        except Exception as e:
            logger.warning("database_metadata_create_tables_warning", error=str(e))
    ...
    async def delete_user_by_email(self, email: str) -> bool:
        ...
        with Session(self.engine) as session:
            ...
            for chat_sess in user_sessions:
    ...
    async def get_user_sessions(self, user_id: int) -> List[ChatSession]:
        ...
        with Session(self.engine) as session:
            statement = (
                select(ChatSession).where(col(ChatSession.user_id) == user_id).order_by(col(ChatSession.created_at).desc())
            )
    ...
    async def save_evolution_pair(self, pair: EvolutionPreferencePair) -> EvolutionPreferencePair:
        """Save a new DPO preference pair / evolution trace.

        Args:
            pair: The EvolutionPreferencePair record to save.

        Returns:
            EvolutionPreferencePair: The saved pair with primary key assigned.
        """
        with Session(self.engine) as session:
            session.add(pair)
            session.commit()
            ...
            logger.info(
                "evolution_pair_saved",
                session_id=pair.session_id,
                domain_topic=pair.domain_topic,
                score=pair.critic_score,
            )
            return pair
    ...
    async def get_evolution_pairs(
        ...
        """Query evolution preference pairs.

        Args:
            limit: Maximum items to return.
            offset: Pagination offset.
            domain_topic: Optional filter by topic.

        Returns:
            List[EvolutionPreferencePair]: List of matching records.
        """
        with Session(self.engine) as session:
            statement = select(EvolutionPreferencePair)
            if domain_topic:
                statement = statement.where(col(EvolutionPreferencePair.domain_topic) == domain_topic)
            statement = statement.order_by(col(EvolutionPreferencePair.created_at).desc()).offset(offset).limit(limit)
    ...
    async def count_evolution_pairs(self, domain_topic: Optional[str] = None) -> int:
        ...
        Args:
            domain_topic: Optional topic filter.

        Returns:
            int: Total count.
        """
        with Session(self.engine) as session:
            statement = select(func.count(col(EvolutionPreferencePair.id)))
            if domain_topic:
                statement = statement.where(col(EvolutionPreferencePair.domain_topic) == domain_topic)
            count_val = session.exec(statement).one()
