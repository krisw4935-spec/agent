...
class Evaluator:
    ...
    def __fetch_traces(self) -> list[TraceWithDetails]:
        ...
        try:
            ...
            if settings.EVALUATION_TRACE_TAG:
                tag = settings.EVALUATION_TRACE_TAG
                traces_without_scores = [
                    trace
                    for trace in traces_without_scores
                    if tag in (getattr(trace, "tags", None) or [])
                ]
                logger.info(
                    "traces_filtered_by_tag",
                    tag=tag,
                    count=len(traces_without_scores),
                )
