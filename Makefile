...
eval-regression:
	@$(call run_with_env,uv run --group test python -m evals.run_regression --mode api)

eval-regression-agent:
	@$(call run_with_env,python -m evals.run_regression --mode agent)
...
# ---------------------------------------------------------------------------
# Docker — full stack (API + DB + Prometheus + Grafana + Langfuse)
# ---------------------------------------------------------------------------
stack-up:
	$(call load_env_file)
...
# ---------------------------------------------------------------------------
# Docker — Langfuse cluster only
# ---------------------------------------------------------------------------
langfuse-up:
	$(call load_env_file)
	@APP_ENV=$(ENV) $(DOCKER_COMPOSE) --env-file .env.$(ENV) up -d db valkey clickhouse minio minio-create-bucket langfuse-web langfuse-worker

langfuse-down:
	$(call load_env_file)
	@APP_ENV=$(ENV) $(DOCKER_COMPOSE) --env-file .env.$(ENV) stop langfuse-web langfuse-worker clickhouse minio minio-create-bucket

langfuse-logs:
	$(call load_env_file)
	@APP_ENV=$(ENV) $(DOCKER_COMPOSE) --env-file .env.$(ENV) logs -f langfuse-web langfuse-worker clickhouse minio
...
help:
	...
	@echo "  eval-regression      Seed 8 math cases via API + run Langfuse eval"
	@echo "  eval-regression-agent  Seed cases via direct LangGraph + eval"
	@echo "  eval-regression-cases  Seed cases only (no eval)"
	@echo ""
	@echo "Code quality:"
	...
	@echo ""
	@echo "Docker (Langfuse cluster):"
	@echo "  langfuse-up          Start self-hosted Langfuse cluster"
	@echo "  langfuse-down        Stop Langfuse cluster"
	@echo "  langfuse-logs        Tail Langfuse cluster logs"
	@echo ""
	@echo "Docker (full stack — includes Prometheus + Grafana + Langfuse):"
